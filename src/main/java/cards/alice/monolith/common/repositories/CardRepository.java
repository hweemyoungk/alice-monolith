...
public interface CardRepository extends JpaRepository<Card, Long> {
    ...
    @Query("select c from Card c where (:customerId is null or c.customerId = :customerId) and (coalesce(:ids, null) is null or c.id in :ids)")
    Set<Card> findByCustomerIdAndIdIn(@Param("customerId") @Nullable UUID customerId, @Param("ids") @Nullable Collection<Long> ids);
}